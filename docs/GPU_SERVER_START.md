...
| 机器 | Config | 说明 |
|------|--------|------|
| **GPU** `rl_server` | `configs/plug_insertion_gpu.yaml` | VLA / rl_token / online_rl / inference；`start_gpu_rl_server.sh` **默认** |
| **工控机** `actor_loop` | `configs/plug_insertion.yaml` | 相机 / reset / gpu_server / deoxys |
...
**启动后验证**（即使终端只显示 Transformer UserWarning，也可先 ping）：

```bash
ss -tlnp | grep 8765
# python ping → buffer_size / train_steps / inference_mode
```

**在线 ckpt 位置**（Stage-3 开始后）：

```
checkpoints/rl_actor.pt
checkpoints/rl_critic.pt
checkpoints/online_rl/rl_*_step*.pt
```

> ⚠️ 重启 server **会清空内存 replay buffer**；权重从上述 ckpt 重载。

**错误示例**（勿用）：

```bash
bash scripts/run_rl_server.sh configs/plug_insertion.yaml   # 工控机 yaml，缺 rl_token
```
