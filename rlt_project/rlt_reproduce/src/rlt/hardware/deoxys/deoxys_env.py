...
class DeoxysEnv:
    ...
    def __init__(self, cfg: DeoxysEnvConfig):
        ...
        if cfg.backend == "deoxys":
            ...
            if cfg.controller_cfg_name:
                user_cfg = YamlConfig(str(Path(config_root) / cfg.controller_cfg_name)).as_easydict()
                self._controller_cfg = user_cfg

            joint_cfg_path = Path(config_root) / cfg.joint_controller_cfg_name
            self._joint_controller_cfg = YamlConfig(str(joint_cfg_path)).as_easydict()
            osc_pos_path = Path(config_root) / "osc-position-controller.yml"
    ...
    def _apply_demo_reset(self) -> None:
        ...
        print(
            ...
            f"(demo_index={sample['demo_index']}, pool={len(self._demo_sampler)})"
        )
        ...
        if self._demo_safety and self._demo_safety.gripper_hold_closed:
    ...
    def reset(self) -> np.ndarray:
        ...
        if self.cfg.use_demo_reset:
            self._apply_demo_reset()
        else:
            self._pose_ready = True
            self._last_reset_info = {"demo_reset": False}

        if self.cfg.gripper_latch:
            self._gripper_latched = True
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, dict]:
        ...
        self._step += 1
        info = {"step": self._step, **self._last_reset_info}
    ...
    def from_rlt_config(cls, raw: dict, *, rlt_root: Path | None = None) -> "DeoxysEnv":
        ...
        demo_path = dc.get("demo_reset_path") or raw.get("paths", {}).get("episodes_dir", "")
        if demo_path and rlt_root is not None and not Path(demo_path).is_absolute():
            demo_path = str((rlt_root / demo_path).resolve())
