# Adapted from RLinf rlinf/envs/realworld/common/gripper/robotiq_gripper.py
# Robotiq 2F-85 / 2F-140 via Modbus RTU (USB-RS485).
...
class RobotiqGripper:
    """Robotiq 2F controlled via Modbus RTU on a serial port."""
    ...
    def __init__(
        ...
        self._logger = logging.getLogger("RobotiqGripper")
        self._port = port
        ...
        self._activate()
        self._logger.info(
            "Robotiq gripper activated on %s (slave=0x%02X, max_width=%sm)",
            port,
            slave_id,
            max_width,
        )
    ...
    def _activate(self) -> None:
        ...
        for _ in range(50):
            ...
            if status is not None and status["gSTA"] == 0x03:
                ...
                return
        raise RuntimeError(f"Robotiq gripper on {self._port} did not activate within 5 s")
    ...
    def _read_status(self) -> Optional[dict]:
        ...
        if resp.isError():
            self._logger.warning("Robotiq Modbus read error: %s", resp)
            return None
