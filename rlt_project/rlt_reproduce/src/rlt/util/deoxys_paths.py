...
def smq_root_from_rlt(rlt_root: Path | None = None) -> Path:
    ...
    root = rlt_root or Path(__file__).resolve().parents[3]
...
def resolve_deoxys_paths(
    *,
    ...
    for candidate in (Path(env_root) if env_root else None, vendored, legacy):
    ...
    raise FileNotFoundError(
        f"deoxys not found. Run: bash {smq}/scripts/robot/copy_deoxys_to_smq.sh "
...
def apply_deoxys_paths(raw: dict, *, smq_root: Path | None = None) -> dict:
    ...
    root, cfg = resolve_deoxys_paths(smq_root=smq_root)
