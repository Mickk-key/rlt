...
NOTE — NOT the runtime copy: at runtime the robot imports the ``smq&jgy/src`` copy of
this module, which is placed FIRST on ``PYTHONPATH`` by ``smq&jgy/scripts/_env.sh`` and
overrides this file. That copy additionally carries robot reset/camera bootstrap logic.
The Phase 4 transition logic (``build_chunk_transitions`` / ``run_episode`` /
``_validate_chunk_transition``) is kept byte-identical between the two copies — apply any
change to that logic in BOTH files. Only the ``main()`` reset/camera bootstrap and
``Path(__file__).parents[N]`` depths differ (they must, different directory depths).
